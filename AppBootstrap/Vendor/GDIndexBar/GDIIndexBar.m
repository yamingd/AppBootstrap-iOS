...
#define kDefaultBarBackgroundWidth 15.f
...
#define kStandardButtonWidth 15.f
...
+ (void)initialize
{
    if (self == [GDIIndexBar class]) {
        ...
        if ([self isOS7OrLater]) {
            UIOffset offset = UIOffsetMake(0.f, 0.f);
...
- (void)reload
{
    ...
    _lineHeight = [@"0" sizeWithAttributes:attr].height;
...
- (void)drawRect:(CGRect)rect
{
    ...
    for (int i = 0; i < indexCount; i++) {
        ...
        // draw normal color
//        [self.textColor set];
        [text drawInRect:CGRectMake(point.x, point.y, textSize.width, _lineHeight) withAttributes:attr];
...
- (UIColor *)textColor
{
    ...
    if ([GDIIndexBar isOS7OrLater]) {
        _textColor = self.superview.tintColor;
        return _textColor;
    }
